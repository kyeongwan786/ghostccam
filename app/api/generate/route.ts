...
Add exactly one realistic but unmistakably supernatural apparition as the only meaningful visual change, following the selected composition variant. It must look like a real ghost accidentally captured by the same camera in the same room: anatomically connected but unsettling, with realistic hair, pale skin, fabric, depth, occlusion, contact shadows, ambient light, focus falloff, motion blur, compression artifacts, and sensor grain that match the source photo. Give it restrained ghost cues such as cool pallor, subtly desaturated tones, an unnatural still gaze, faint edge translucency, slight motion ghosting, or a physically impossible stillness, while keeping real photographic texture. It must be visible at normal viewing size and clearly feel supernatural, not like an ordinary healthy person.

Anatomy and composition requirement: never show a floating head, detached neck, face-only peek, or a head sticking out by itself. The head must connect naturally to visible shoulders and an upper torso, or to a complete body when the placement allows it. If furniture or a wall occludes the apparition, show believable connected body parts continuing behind the occluder with natural depth and shadow. Do not crop the apparition at the neck; do not make it look like a severed mannequin or a random portrait pasted into the background.
...
const PHOTOREALISM_RULES =
  "Photorealism and supernatural balance: the original image must remain visually dominant and unchanged; the connected apparition must inherit the source camera's lighting, white balance, focus, grain, compression, perspective, and occlusion while retaining subtle spectral cues. It should look like a real ghost captured in a real photograph, never an ordinary person, floating head, illustration, or standalone generated character.";
